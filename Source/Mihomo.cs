    ...
    internal interface Mihomo
    {
        public static async void GetResources(int Type)
        {
            ...
            while (true)
            {
                ...
                {
                    Program.Message($"成功更新：{Total}组数据"); break;
                }
                else if (Program.Message($"重试资源：{Pages.Count}/{Total}组数据 {Fails.Count}项图片"))
                {
                    if (Fails.Count > 0)
                    {
        ...
        public static async Task<List<string>> GetWikiPage(int Id)
        {
            ...
            List<string> Download = [$"{Id}-{Name}", "Icon", Icon];
            ...
            {
                case "角色":
                    GetWikiRole(ref Download, RoleConten, Filter, Contents, Id, Name);
                    break;
                case "光锥":
                    GetWikiEquip(ref Download, Contents, Id, Name);
                    break;
                case "遗器":
                case "位面饰品":
                    GetWikiRelic(ref Download, Contents, Id, Name);
                    break;
                case "敌人图鉴":
                    await Task.Run(() => GetWikiEnemy(ref Download, Contents, Id, Name));
                    break;
                default:
        ...
        private static void GetWikiRole(ref List<string> Download, JsonElement RoleContent, string Filter, List<string> Contents, int Id, string Name)
        {
            Role role = new()
            {
                Id = Id,
        ...
        private static void GetWikiEquip(ref List<string> Download, List<string> Contents, int Id, string Name)
        {
            Equip equip = new()
            {
                Id = Id,
            ...
            equip.Skill.Name = Simple.Get_Html_Text(Tags[0]);
            equip.Skill.Text = Simple.Get_Html_Text(Tags[1]);
            MatchCollection Mats = Regex.Matches(equip.Skill.Text, @"【([\d%/.]+)】");
            foreach (Match M in Mats)
            {
                equip.Skill.Values.Add([.. M.Groups[1].Value.Split('/')]);
            }
        ...
        private static void GetWikiRelic(ref List<string> Download, List<string> Contents, int Id, string Name)
        {
            Relic relic = new()
            {
                Id = Id,
        ...
        private static void GetWikiEnemy(ref List<string> Download, List<string> Contents, int Id, string Name)
        {
            Enemy enemy = new()
            {
                Id = Id,
