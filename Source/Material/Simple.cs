...
namespace SR_DMG.Source.Material
{
    interface Simple
    {
        ...
        static string RootFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), App);
        const string App = "SR-DMG";
        const string Log = "logs.txt";
        const string About = "About.json"; // 描述文件
        const string Resources = "Resources"; // 资源目录
        static string Roles = Path.Combine(Resources, "Roles"); // 角色
        static string Equips = Path.Combine(Resources, "Equips"); // 光锥
        static string Relics = Path.Combine(Resources, "Relics"); // 侵蚀隧洞
        static string Enemy = Path.Combine(Resources, "Enemy"); // 敌对物种
        const string Ranks = "Ranks"; // 星魂
        const string Trace = "Trace"; // 行迹
        static GainView? Role_Skills;
        static BitmapImage Not_Find_Image = (BitmapImage)Application.Current.Resources["Icon_Not_Image"];
        ...
        static JsonSerializerOptions JsonOptions = new() { WriteIndented = true, Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping };
        static Color[] Element_Colors = [
