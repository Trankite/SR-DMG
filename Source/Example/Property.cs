    ...
    public class Property<TKey, TValue>(Func<TKey, TValue> GetFunc, Action<TKey, TValue> SetAction)
    {
        ...
        public static void Initialize<TSetKey, TSetValue>(Dictionary<string, Property<TSetKey, TSetValue>> Properties, Dictionary<string, string> Keys)
        {
            ...
            foreach (PropertyInfo Prop in Type.GetProperties())
            {
                ...
                ParameterExpression ProptParam = Expression.Parameter(Type);
                ParameterExpression ValueParam = Expression.Parameter(typeof(TSetValue));
                MemberExpression PropAccess = Expression.Property(Expression.Convert(ProptParam, Type), Prop);
                Properties[PropertName] = new(Expression.Lambda<Func<TSetKey, TSetValue>>(PropAccess, ProptParam).Compile(),
                    Expression.Lambda<Action<TSetKey, TSetValue>>(Expression.Assign(PropAccess, ValueParam), ProptParam, ValueParam).Compile());
            }
        }
    }
}